//==========================================================================================
//
//		OpenNETCF.IO.StreamInterfaceDriver
//		Copyright (c) 2003, OpenNETCF.org
//
//		This library is free software; you can redistribute it and/or modify it under 
//		the terms of the OpenNETCF.org Shared Source License.
//
//		This library is distributed in the hope that it will be useful, but 
//		WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or 
//		FITNESS FOR A PARTICULAR PURPOSE. See the OpenNETCF.org Shared Source License 
//		for more details.
//
//		You should have received a copy of the OpenNETCF.org Shared Source License 
//		along with this library; if not, email licensing@opennetcf.org to request a copy.
//
//		If you wish to contact the OpenNETCF Advisory Board to discuss licensing, please 
//		email licensing@opennetcf.org.
//
//		For general enquiries, email enquiries@opennetcf.org or visit our website at:
//		http://www.opennetcf.org
//
//==========================================================================================

using System;
using System.Runtime.InteropServices;
using Neolix.Device;

namespace Neolix.Device.Tools
{
    /// <summary>
    /// Use this abstract base class to create wrappers around Stream Interface
    /// Drivers that are not supported by the CF
    /// <para><b>New in v1.1</b></para>
    /// </summary>
    public class StreamInterfaceDriver : IDisposable
    {
        protected IntPtr m_hPort = IntPtr.Zero;
        private string m_portName = null;

        public IntPtr Handle
        {
            get
            {
                return m_hPort;
            }
        }

        /// <summary>
        /// Create an instance of the StreamInterfaceDriver class
        /// </summary>
        /// <param name="portName">Name of port (prefix and index) to open</param>
        public StreamInterfaceDriver(string portName)
        {
            m_portName = portName;
        }

        ~StreamInterfaceDriver()
        {
            this.Dispose();
        }

        /// <summary>
        /// Open the driver
        /// <see cref="FileAccess"/>
        /// <see cref="FileShare"/>
        /// </summary>
        /// <param name="access">File Access (Read, Write or Both)</param>
        /// <param name="share">Share Mode (Read, Write or both)</param>
        protected void Open(FileAccess access, FileShare share)
        {
            m_hPort = FileEx.CreateFile(m_portName, access, share, FileCreateDisposition.OpenExisting, 0);

            if ((int)m_hPort == -1)
            {
                throw new WinAPIException("Cannot open driver", Marshal.GetLastWin32Error());
            }
        }

        /// <summary>
        /// Read data from the driver
        /// </summary>
        /// <param name="bytesToRead">The number of bytes requested.</param>
        /// <returns>A byte array returned by the driver</returns>
        public byte[] Read(int bytesToRead)
        {
            byte[] buffer = new byte[bytesToRead];
            int read = 0;

            FileEx.ReadFile(m_hPort, buffer, bytesToRead, ref read);

            return buffer;
        }

        public byte[] Read(int bytesToRead, out int bytesRead)
        {
            byte[] buffer = new byte[bytesToRead];
            bytesRead = 0;

            FileEx.ReadFile(m_hPort, buffer, bytesToRead, ref bytesRead);

            return buffer;
        }

        /// <summary>
        /// Write data to the driver
        /// </summary>
        /// <param name="data">Data to write</param>
        /// <returns>Number of bytes actually written</returns>
        public int Write(byte[] data)
        {
            return Write(data, data.Length);
        }

        /// <summary>
        /// Write data to the driver
        /// </summary>
        /// <param name="data">Data to write</param>
        /// <param name="bytesToWrite">Number of bytes from passed in buffer to write</param>
        /// <returns>Number of bytes actually written</returns>
        public int Write(byte[] data, int bytesToWrite)
        {
            int written = 0;

            FileEx.WriteFile(m_hPort, data, bytesToWrite, ref written);

            return written;
        }

        /// <summary>
        /// Call a device specific IOControl
        /// </summary>
        /// <param name="controlCode">The IOCTL code</param>
        /// <param name="inData">Data to pass into the IOCTL</param>
        /// <param name="outData">Data returned by the IOCTL</param>
        public void DeviceIoControl(uint controlCode, byte[] inData, byte[] outData)
        {
            int xfer = 0;
            int inlen = (inData == null) ? 0 : inData.Length;
            int outlen = (outData == null) ? 0 : outData.Length;

            if (FileEx.DeviceIoControlCE(m_hPort, controlCode, inData, inlen, outData, outlen, ref xfer, IntPtr.Zero) ==
                0)
            {
                throw new WinAPIException("IOControl call failed", Marshal.GetLastWin32Error());
            }
        }

        /// <summary>
        /// Call a device specific IOControl
        /// </summary>
        /// <param name="controlCode">The IOCTL code</param>
        public void DeviceIoControl(uint controlCode)
        {
            DeviceIoControl(controlCode, null, null);
        }

        /// <summary>
        /// This function moves the file pointer of an open file
        /// <seealso cref="FileEx.MoveMethod"/>
        /// </summary>
        /// <param name="distance">Bytes to move - a positive number moves forward, a negative moves backward</param>
        /// <param name="seekFrom">Starting position from where distance is measured</param>
        /// <returns>New file position</returns>
        /// <remarks>The current file position can be queried using seekFrom(0, MoveMethod.CurrentPosition)</remarks>
        public int Seek(int distance, FileEx.MoveMethod seekFrom)
        {
            int dist = FileEx.SetFilePointerCE(m_hPort, distance, 0, seekFrom);

            if (dist == 0)
            {
                throw new WinAPIException("Seek Failed", Marshal.GetLastWin32Error());
            }

            return dist;
        }

        /// <summary>
        /// Close the driver
        /// </summary>
        public void Close()
        {
            if (m_hPort != IntPtr.Zero)
            {
                FileEx.CloseHandle(m_hPort);
                m_hPort = IntPtr.Zero;
            }
        }

        public bool IsOpen
        {
            get
            {
                return m_hPort != IntPtr.Zero && m_hPort != (IntPtr) (-1);
            }
        }

        public void Dispose()
        {
            Close();
        }
    }
}